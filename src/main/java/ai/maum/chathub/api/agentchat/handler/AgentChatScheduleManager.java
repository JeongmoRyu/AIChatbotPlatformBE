...
@Slf4j
public class AgentChatScheduleManager {
    ...
    public AgentChatScheduleManager(int poolSize, long maxExecutionTime) {
...
//        this.taskStartTimes = new ConcurrentHashMap<>();
    ...
    public ScheduledFuture<?> scheduledFuture(Runnable task, long initialDelay, long period, TimeUnit unit) {
        ...
        try {
            AgentChatScheduleTask agentChatScheduleTask = (AgentChatScheduleTask) task;
            scheduleObject = new ScheduleObject(System.currentTimeMillis(), agentChatScheduleTask.getMemberId(), agentChatScheduleTask.getChatbotId(), agentChatScheduleTask.getRoomId());
