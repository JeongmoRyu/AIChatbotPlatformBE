...
import ai.maum.chathub.api.agentchat.dto.elastic.*;
import ai.maum.chathub.api.agentchat.dto.maumai.*;
...
import ai.maum.chathub.api.agentchat.handler.AgentChatSocketIOHandler;
...
//                String title = answer != null ? answer.substring(0, answer.length() > 20 ? 20 : answer.length() - 1) : null;
                String title = answer != null ? answer.substring(0, Math.min(answer.length(), 20)) : null;
                chatroomService.setChatroomTitle(chatroomDetailVO.getRoomId(), title);
            }
        }
    }
    // 첫 질문으로 chatroom 업데이트
    @Transactional
    public void setChatroomTitleFirstQ(ChatroomDetailVO chatroomDetailVO, String answer) {
        if (chatroomDetailVO != null || chatroomDetailVO.getRoomId() != null
                || chatroomDetailVO.getRoomId() > 0) {
//
            if (chatroomDetailVO.getRoomId() != null && chatroomDetailVO.getRoomId() > 0) {
//                String title = answer != null ? answer.substring(0, answer.length() > 20 ? 20 : answer.length() - 1) : null;
                String title = answer != null ? answer.substring(0, Math.min(answer.length(), 20)) : null;
                chatroomService.setChatroomTitleFirstQ(chatroomDetailVO.getRoomId(), title);
            }
        }
    }
    // 질문에 따라 chatroom 시간 업데이트
    @Transactional
    public void updateChatroomTimestamp(Long roomId) {
        if (roomId == null || roomId <= 0) {
            return;
        }
        chatroomService.updateChatroomTimestamp(roomId);
    }
    ...
    public void sendSocketMessage(String roomId, String title, String content) {
        try {
            if (roomId == null || roomId.isEmpty()) {
                ...
                agentChatSocketIOHandler.sendMessageToSession(roomId, title, content);
            }
        } catch (Exception e) {
...
//    }
    ...
    public Flux<String> startChat(ChatbotVO chatbot, Long chatroomId, String roomId, List<ChatMessage> messages) throws Exception {
        ...
        // 첫 질문 값으로 타이틀에 넣어주기  chatroom title
        setChatroomTitle(chatroomDetailVO, content);
